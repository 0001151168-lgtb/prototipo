...
// ===================== GERAR POP EM PDF =====================
function gerarPOPContent() {
    if (receitaAtualId === null) {
        showNotification('Nenhuma receita selecionada. Visualize uma receita primeiro.', 'warning');
        return null;
    }
    ...
    // Monta o HTML do POP
    let html = `
        <div style="font-family: Arial, sans-serif; max-width: 800px; margin: 0 auto; padding: 20px; background: #fff; color: #333;">
            <div style="text-align: center; border-bottom: 3px solid #2c3e50; padding-bottom: 10px; margin-bottom: 20px;">
                <h1 style="color: #2c3e50; margin: 0;">PROCEDIMENTO OPERACIONAL PADRÃO (POP)</h1>
                <h2 style="color: #3498db; margin: 5px 0;">Ficha Técnica de Preparo</h2>
            </div>

            <div style="margin-bottom: 20px;">
                <div style="display: flex; justify-content: space-between; flex-wrap: wrap;">
                    <div><strong>Código:</strong> ${receita.codigo}</div>
                    <div><strong>Data de Geração:</strong> ${new Date().toLocaleDateString('pt-BR')}</div>
                </div>
                <div style="display: flex; justify-content: space-between; flex-wrap: wrap; margin-top: 5px;">
                    <div><strong>Receita:</strong> ${receita.nome}</div>
                    <div><strong>Categoria:</strong> ${receita.categoria}</div>
                </div>
            </div>

            <div style="background: #f8f9fa; padding: 15px; border-radius: 8px; margin-bottom: 20px;">
                <h3 style="margin-top: 0; color: #2c3e50;">Descrição</h3>
                <p style="margin: 5px 0;">${receita.descricao || 'Não informada'}</p>
                <div style="display: flex; flex-wrap: wrap; gap: 20px; margin-top: 10px;">
                    <div><strong>Porções:</strong> ${receita.porcoes}</div>
                    <div><strong>Tempo de Preparo:</strong> ${receita.tempoPreparo} min</div>
                    <div><strong>Lucro Esperado:</strong> ${receita.lucro}%</div>
                </div>
            </div>

            <div style="margin-bottom: 20px;">
                <h3 style="color: #2c3e50; border-bottom: 2px solid #ecf0f1; padding-bottom: 5px;">Ingredientes</h3>
                <table style="width: 100%; border-collapse: collapse; margin-top: 10px;">
                    <thead>
                        <tr style="background: #2c3e50; color: white;">
                            ...
                            <th style="padding: 8px; text-align: center;">Unidade</th>
                            <th style="padding: 8px; text-align: right;">Custo Unitário</th>
    ...
    receita.ingredientes.forEach(ing => {
        const item = buscarItemPorId(ing.itemId);
        const nome = item ? item.nome : 'Item não encontrado';
        const custoUnit = ing.custoUnitario || 0;
        const custoTotal = ing.quantidade * custoUnit;
        html += `
            <tr>
                <td style="padding: 6px; border-bottom: 1px solid #ecf0f1;">${nome} ${ing.observacao ? `(${ing.observacao})` : ''}</td>
                <td style="padding: 6px; border-bottom: 1px solid #ecf0f1; text-align: center;">${ing.quantidade}</td>
                <td style="padding: 6px; border-bottom: 1px solid #ecf0f1; text-align: center;">${ing.unidade}</td>
                <td style="padding: 6px; border-bottom: 1px solid #ecf0f1; text-align: right;">R$ ${custoUnit.toFixed(2)}</td>
                <td style="padding: 6px; border-bottom: 1px solid #ecf0f1; text-align: right;">R$ ${custoTotal.toFixed(2)}</td>
            </tr>
        `;
    });
    ...
    html += `
                    ...
                    <tfoot>
                        <tr>
                            <td colspan="4" style="padding: 8px; text-align: right; font-weight: bold;">Custo Total da Receita:</td>
                            <td style="padding: 8px; text-align: right; font-weight: bold;">R$ ${receita.custoTotal.toFixed(2)}</td>
                        </tr>
                        <tr>
                            <td colspan="4" style="padding: 8px; text-align: right; font-weight: bold;">Custo por Porção:</td>
                            <td style="padding: 8px; text-align: right; font-weight: bold;">R$ ${receita.custoPorcao.toFixed(2)}</td>
                        </tr>
                    </tfoot>
                </table>
            </div>
            ...
            <div style="margin-bottom: 20px;">
                <h3 style="color: #2c3e50; border-bottom: 2px solid #ecf0f1; padding-bottom: 5px;">Modo de Preparo</h3>
                <ol style="padding-left: 20px; margin-top: 10px;">
    `;

    receita.instrucoes.forEach(inst => {
        html += `<li style="margin-bottom: 5px;">${inst}</li>`;
    });
    ...
    html += `
            ...
            <div style="margin-bottom: 20px;">
                <h3 style="color: #2c3e50; border-bottom: 2px solid #ecf0f1; padding-bottom: 5px;">Informações Nutricionais (por porção)</h3>
                <div style="display: flex; flex-wrap: wrap; gap: 20px; margin-top: 10px;">
                    <div><strong>Calorias:</strong> ${receita.calorias || 0} kcal</div>
                    <div><strong>Proteínas:</strong> ${receita.proteinas || 0}g</div>
                    <div><strong>Carboidratos:</strong> ${receita.carboidratos || 0}g</div>
                    <div><strong>Gorduras:</strong> ${receita.gorduras || 0}g</div>
                </div>
            </div>

            <div style="text-align: center; font-size: 12px; color: #7f8c8d; border-top: 1px solid #ecf0f1; padding-top: 10px; margin-top: 20px;">
                Documento gerado automaticamente pelo Sistema Inventario Ágil em ${new Date().toLocaleString('pt-BR')}
            </div>
        </div>
    `;
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // ===================== BOTÃO GERAR POP =====================
    document.getElementById('gerarPOPBtn').addEventListener('click', function() {
        const content = gerarPOPContent();
        if (!content) return;

        // Cria um elemento temporário para renderizar o PDF
        ...
        const opt = {
            margin:        [10, 10, 10, 10],
            filename:     `POP_${receitaAtualId}_${new Date().toISOString().slice(0,10)}.pdf`,
        ...
        html2pdf().set(opt).from(element).save().then(() => {
            ...
            showNotification('POP gerado com sucesso!', 'success');
        }).catch(err => {
            document.body.removeChild(element);
            showNotification('Erro ao gerar POP: ' + err.message, 'error');
        });
    });
